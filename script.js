...
function updateJoinButtonText() {
  const joinBtn = document.getElementById('joinSpaceBtn');
  if (joinBtn) {
    joinBtn.textContent = currentTheme === 'garden' ? '加入花园' : '加入星系';
  }
}
...
function updateCreateButtonText() {
  ...
  if (createBtn) {
    createBtn.textContent = currentTheme === 'garden' ? '+ 创建花园' : '+ 创建星系';
  }
}
...
function updateGroupList() {
  ...
  let html = `
    <div class="group-item ${currentGroup === 'default' ? 'active' : ''}">
      <div class="group-content" onclick="selectGroup('default')">
...
function joinByInviteCode() {
  ...
  const groups = loadData('groups', {});
  const foundGroup = Object.values(groups).find(group => group.inviteCode && group.inviteCode.toUpperCase() === inviteCode);
...
function joinGroupViaLink() {
  ...
  if (inviteCode) {
    const groups = loadData('groups', {});
    const foundGroup = Object.values(groups).find(group => group.inviteCode && group.inviteCode.toUpperCase() === inviteCode.toUpperCase());
...
function initProfile() {
  ...
  if (!userData.bio) {
    userData.bio = '点击编辑个人简介...';
    saveData('currentUser', userData);
  }
  
  document.getElementById('profileName').textContent = userData.name || '我的名字';
...
function editProfile() {
  const newName = prompt('请输入你的名字:', document.getElementById('profileName').textContent);
  if (newName && newName.trim()) {
    let userData = loadData('currentUser', { id: 'user_' + Date.now(), name: '我' });
